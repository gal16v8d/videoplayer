...
/**
 * Implements a thread for take the snapshot, because if the snapshot run on application thread you
 * will no able to use the menu or properly close the application until the snapshot over, implement
 * as a thread solves that issue.
 *
...
public class SnapThread extends Thread {
  ...
  /**
   * This method runs the thread for take the snapshots and put it on the folder. Listing
   * properties:
   *
   * <ul>
   ...
   *   <li>The name of the snapshot is {@link ConstantsPlayer#SNAPSHOT_PREFIX} and number of
   *       capture.
   *   <li>The image format is {@link ConstantsPlayer#SNAPSHOT_EXT}
   * </ul>
   *
   * <p>Once the method finish the video will stop and you can select another video.
   *
   * @since 1.0
   * @see ConstantsPlayer
   */
