 ...
 *
 * <p>Requisites Taken: If new video in folder exists and convert it to images. Convert entire video
 * to 250 images which should be distributed equally through out the video. (not the 1st 250 frames
 * but to pick 250 frames from the entire video) Store the 250 images in another folder. The app
 * should not require user input. detect if new video came in, convert that to 250 images in another
 * folder.
 *
 * <p>Important: The Libraries used in this project are in the pom.xml at this project.For a good
 * use you will need:
 *
 * <ul>
...
public class VlcPlayerMain {
  ...
  private static String loadSearchPath(String[] args) {
    return args.length == 1 && args[0] != null
        ? ConstantsPlayer.validateRoute(args[0])
