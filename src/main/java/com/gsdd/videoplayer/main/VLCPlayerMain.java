 ...
 * <p>
 * Requisites Taken: If new video in folder exists and convert it to images. Convert entire video to
 * 250 images which should be distributed equally through out the video. (not the 1st 250 frames but
 * to pick 250 frames from the entire video) Store the 250 images in another folder. The app should
 * not require user input. detect if new video came in, convert that to 250 images in another
 * folder.
 * <p>
 * Important: The Libraries used in this project are in the pom.xml at this project.For a good use
 * you will need:
 * <ul>
 * <li>Java in your PC (x86 or x64)
 * <li>VLC Media Player (Minimum version 2.1.5) NOTE: If java is (x86) I recommend use a VLC (x86)
 * too AND if java is (x64) I recommend use a VLC (x64) too.
 * </ul>
 * <p>
 * 
 * <ul>
...
public class VLCPlayerMain {
  ...
  public static void main(final String[] args) {
    ...
    SwingUtilities
        .invokeLater(() -> new VLCPlayerView().getController().setSleep(loadCaptureTime(args)));
  }
   ...
   * 
   * @since 1.0
   * @param args The input args in execution.
